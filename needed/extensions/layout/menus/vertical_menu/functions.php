	...
	function load_pages_v_menu($menu_array) {
		...
		for ($i = 0; $i <= count($menu_array); $i++) {
			...
			if (!empty($menu_array[$i]['id'])) {
				...
				$out .=  '<li class="'.$class.'" id="menu-' . $menu_array[$i]['id'] .'"> <a href="' . $CONF['url'] . $page_data['prefix'] .'"> '. $page_data['title'] .' </a> ';
					
				if (isset($menu_array[$i]['children'])) {
						
					$children = $menu_array[$i]['children'];
					
					$out .='<ul><li class="" id="menu-' . $menu_array[$i]['id'] .'"> <a href="' . $CONF['url'] . $page_data['prefix'] .'"> '. $page_data['title'] .' </a> </li>';
